...
namespace {
void trim_log(File &file, const std::string &content, unsigned long line_count, unsigned long trim_to_index) {
  ...
  try {
    ...
    for (File &file : files) {
      ...
      if (line_count >= trim_from) {
        trim_log(file, content, line_count, trim_to_index);
      }
    }
  } catch (const std::exception &e) {
    app_log(std::string("Error in log trimmer: ") + e.what());
  }
}